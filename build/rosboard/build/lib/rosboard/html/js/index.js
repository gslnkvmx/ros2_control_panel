...
const RobotCommands = {
  ...
  sendCommand: function(commandName, data = true) {
    ...
    const message = command.handler.call(command, data);
    
    if (currentTransport && currentTransport.ws) {
      const commandMsg = ['c', {
        topic: message.topic,
        msg: message.msg
      }];
      
      currentTransport.ws.send(JSON.stringify(commandMsg));
      console.log(`Sent command: ${commandName}`, message);
      
      // Show feedback to user
      snackbarContainer.MaterialSnackbar.showSnackbar({
        message: `Command ${commandName} sent successfully`,
        timeout: 2000
      });
    } else {
  ...
  addCommand: function(name, config) {
